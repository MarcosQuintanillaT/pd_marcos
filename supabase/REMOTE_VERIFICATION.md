...
- El bucket privado `portafolio-documentos` acepta `application/pdf`, `text/html` e `image/*` y limita cada archivo a 4 MB.
- Se verificaron cargas temporales reales de `text/html` e `image/png`; ambos objetos se eliminaron al finalizar la prueba.
- Las cuatro políticas de `storage.objects` para lectura, subida, reemplazo y eliminación están instaladas.
- La configuración pública de Auth reporta `disable_signup=true`; el registro público permanece desactivado.

...
import { BookOpen, ChevronDown, ExternalLink, LogOut, Menu, PanelLeftClose, ShieldCheck, UserRound, X } from "lucide-react";
    ...
    <aside className="flex h-full w-[310px] max-w-[calc(100vw-1rem)] flex-col bg-[#123b35] text-white">
      <div className="flex h-[86px] items-center justify-between border-b border-white/10 px-5">
        <Link href="/portafolio" className="flex min-w-0 items-center gap-3 rounded-lg">
          <span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-[#c98b3c] text-white"><BookOpen size={21} /></span>
      ...
      <nav className="sidebar-scroll flex-1 overflow-y-auto px-3 py-5" aria-label="Índice oficial del portafolio">
        <Link href="/portafolio" aria-current={pathname === "/portafolio" ? "page" : undefined} className={`mb-3 flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold transition ${pathname === "/portafolio" ? "bg-white text-[#123b35]" : "text-white/75 hover:bg-white/8 hover:text-white"}`}>
                ...
                <div className="ml-[26px] border-l border-white/10 py-1.5 pl-3">
                  ...
                  {section.subsections.map((subsection) => (
                    <div key={subsection.code}>
                      <Link href={sectionHref(section, subsection)} className={`block rounded-lg px-3 py-2 text-xs leading-[1.15rem] transition ${pathname === sectionHref(section, subsection) ? "bg-white/12 font-bold text-white" : "text-white/57 hover:bg-white/7 hover:text-white"}`}>
    ...
    <div className="min-h-screen lg:pl-[310px]">
      ...
      <header className="sticky top-0 z-30 flex h-[72px] items-center justify-between border-b border-[#dedbd0]/80 bg-[#f4f1e8]/88 px-4 backdrop-blur-xl sm:px-7 lg:px-10">
        ...
        <div className="flex items-center gap-3">
          ...
          <a href="https://supabase.com/dashboard" target="_blank" rel="noreferrer" className="hidden size-9 place-items-center rounded-xl text-[#73817d] hover:bg-white sm:grid" aria-label="Abrir Supabase"><ExternalLink size={16} /></a>
        </div>
      </header>

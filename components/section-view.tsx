...
export function SectionView({ section, subsection }: { section: Seccion; subsection?: Subseccion }) {
  ...
  const [partial, setPartial] = useState<Parcial | null>(subsection?.fixedParcial ?? null);
    ...
    try {
      if (demoMode) {
        const matches = DEMO_DOCUMENTS.filter((document) =>
          ...
          && (!partial || document.parcial === partial)
      ...
      if (partial) params.set("parcial", partial);
  ...
  }, [configured, demoMode, isCoverOverview, isLeaf, page, partial, queryText, section, selectedId, status, subsection]);
    ...
    <div className="animate-enter px-4 py-7 sm:px-7 lg:px-10 lg:py-9 xl:px-12">
      ...
      <header className="paper-card relative overflow-hidden rounded-[1.7rem] p-6 sm:p-8">
        ...
        <div className="flex flex-wrap items-start justify-between gap-5 pr-2">
          ...
          {role === "docente" && isLeaf && selected?.estado === "Activo" && <UploadForm section={section} subsection={subsection!} parcial={activePartial} onCreated={(document) => { setDocuments((items) => [document, ...items]); setTotal((value) => value + 1); }} />}
        </div>
      </header>

      {subsection?.supportsParcial && <div className="mt-5 flex flex-wrap items-center gap-2 rounded-2xl border border-[#dedbd0] bg-[#fffdf8]/80 p-3" role="group" aria-label="Filtrar por parcial"><span className="mr-1 px-2 text-[11px] font-extrabold uppercase tracking-wide text-[#5f716c]">Filtrar por parcial</span><button type="button" aria-pressed={partial === null} onClick={() => { setPage(1); setPartial(null); }} className={`rounded-lg px-3.5 py-2 text-xs font-bold ${partial === null ? "bg-[#123b35] text-white" : "bg-[#ece9e1] text-[#5d706a]"}`}>Todos</button>{PARCIALES.map((item) => <button type="button" key={item} aria-pressed={partial === item} onClick={() => { setPage(1); setPartial(item); }} className={`rounded-lg px-3.5 py-2 text-xs font-bold ${partial === item ? "bg-[#123b35] text-white" : "bg-[#ece9e1] text-[#5d706a] hover:bg-[#e3ded2]"}`}>{item} Parcial</button>)}</div>}
      ...
      {isLeaf && <section className="mt-8">
        <div className="mb-4 flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-lg font-bold text-[#294740]">Documentos de esta subsección</h2><p className="mt-1 text-xs text-[#5f716c]">{total} {total === 1 ? "archivo" : "archivos"}{partial ? ` · ${partial} Parcial` : ""}</p></div>{error && <button type="button" onClick={() => void load()} className="flex min-h-11 items-center gap-2 rounded-lg bg-red-50 px-3 text-xs font-bold text-red-700"><RefreshCw size={14} />Reintentar</button>}</div>

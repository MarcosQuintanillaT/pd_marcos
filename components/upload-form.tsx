...
export function UploadForm({ section, subsection, parcial, onCreated }: { section: Seccion; subsection: Subseccion; parcial: Parcial | null; onCreated: (document: Documento) => void }) {
  ...
  function pick(next: File | null) {
    ...
    const validationError = validatePortfolioFile(next, MAX_CLIENT_FILE_BYTES);
    if (validationError) {
      setFile(null);
      return setError(validationError);
    }
    setFile(next);
    if (!title) setTitle(filenameWithoutExtension(next.name));
  }
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (response.ok) { onCreated(json.documento); setOpen(false); setTitle(""); setFile(null); } else setError(json.error ?? "No se pudo subir el archivo");
    setUploading(false);
  }

  return <>
    <button onClick={() => setOpen(true)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#123b35] px-4 py-3 text-xs font-bold text-white shadow-lg shadow-[#123b35]/12 hover:bg-[#1b5148]"><FileUp size={16} />Subir nuevo archivo</button>
    {open && <div className="fixed inset-0 z-[65] grid overflow-y-auto bg-[#0b2824]/65 p-4 backdrop-blur-sm"><form onSubmit={submit} className="animate-enter my-auto max-h-[calc(100dvh-2rem)] w-full max-w-lg justify-self-center overflow-y-auto rounded-3xl bg-[#fffdf8] p-6 shadow-2xl sm:p-7" role="dialog" aria-modal="true" aria-label="Subir archivo">
      ...
      <div className="mt-5"><input ref={input} type="file" accept={FILE_INPUT_ACCEPT} className="sr-only" onChange={(e) => { const selected = e.currentTarget.files?.[0] ?? null; e.currentTarget.value = ""; pick(selected); }} /><button type="button" onClick={() => input.current?.click()} className="flex w-full flex-col items-center rounded-2xl border-2 border-dashed border-[#d7d3c7] bg-[#f6f3eb] px-5 py-7 text-center hover:border-[#c98b3c]"><UploadCloud size={30} className="text-[#c98b3c]" /><strong className="mt-3 text-sm text-[#34544e]">{file ? file.name : "Seleccionar archivo"}</strong><span className="mt-1 text-[11px] text-[#84908c]">PDF, imágenes o HTML · máximo 4 MB</span></button></div>
      {error && <p role="alert" className="mt-4 rounded-xl bg-red-50 px-4 py-3 text-xs text-red-700">{error}</p>}
      <div className="mt-6 flex justify-end gap-2"><button type="button" onClick={() => setOpen(false)} className="min-h-11 rounded-xl px-4 py-2.5 text-xs font-bold text-[#62736e]">Cancelar</button><button disabled={uploading} className="flex min-h-11 items-center gap-2 rounded-xl bg-[#123b35] px-5 py-2.5 text-xs font-bold text-white disabled:opacity-60"><UploadCloud size={15} />{uploading ? "Subiendo…" : "Subir archivo"}</button></div>
    </form></div>}
  </>;
}
